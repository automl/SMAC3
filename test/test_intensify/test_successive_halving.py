...
class TestSuccessiveHalving(unittest.TestCase):
    ...
    def test_process_results_via_sourceid(self):
        ...
        for i in np.random.choice(list(range(10)), 30):
            ...
            self.assertEqual(
                self.SH.intensifier_instances[i].process_results.call_args[1][
                    "run_info"
                ],
                run_info,
            )
            self.assertEqual(
                self.SH.intensifier_instances[i].process_results.call_args[1]["result"],
                result,
            )
            ...
            for j in range(len(self.SH.intensifier_instances)):
                ...
                if self.SH.intensifier_instances[j].process_results.call_args is None:
                    ...
                    all_other_run_infos.append(
                        self.SH.intensifier_instances[j].process_results.call_args[1][
                            "run_info"
                        ]
                    )
    ...
    def test_parallel_same_as_serial_SH(self):
        ...
        self.assertTrue(self.SH._add_new_instance(num_workers=2))
        self.assertTrue(self.SH._add_new_instance(num_workers=2))
        incumbent_psh, inc_perf_psh = self._exhaust_run_and_get_incumbent(
            self.SH, self.rh
        )
...
class Test_SuccessiveHalving(unittest.TestCase):
    ...
    def test_init_1(self):
        ...
        self.assertEqual(
            len(intensifier.inst_seed_pairs), 6
        )  # since instance-seed pairs
    ...
    def test_init_2(self):
        ...
        self.assertEqual(
            len(intensifier.inst_seed_pairs), 1
        )  # since instance-seed pairs
        self.assertEqual(intensifier.initial_budget, 1)
    ...
    def test_top_k_2(self):
        ...
        with self.assertRaisesRegex(ValueError, "Cannot compare configs"):
            intensifier._top_k(
                configs=[self.config2, self.config1, self.config3],
                k=1,
                run_history=self.rh,
            )
    ...
    def test_top_k_4(self):
        ...
        intensifier._update_stage(self.rh)
        self.assertEqual(
            intensifier.fail_chal_offset, 1
        )  # we miss one challenger for this round
        configs = intensifier._top_k(configs=[self.config1], k=2, run_history=self.rh)
        self.assertEqual(configs, [self.config1])
        ...
        self.assertEqual(
            intensifier.stage, 0
        )  # going back, since there are not enough to advance
    ...
    def test_eval_challenger_capping_2(self):
        ...
        self.assertEqual(
            intensifier.fail_chal_offset, 1
        )  # 2 configs expected, but 1 failure
        ...
        self.assertEqual(
            len(
                self.rh.get_runs_for_config(self.config4, only_max_observed_budget=True)
            ),
            4,
        )
        self.assertEqual(intensifier.sh_iters, 1)
    ...
    def test_eval_challenger_3(self):
        ...
        self.assertEqual(
            intensifier.configs_to_run, [self.config1]
        )  # Incumbent is promoted to the next stage
        self.assertEqual(intensifier.stage, 1)
    ...
    def test_incumbent_selection_default(self):
        ...
        inc = intensifier._compare_configs(
            challenger=self.config1, incumbent=inc, run_history=self.rh, log_traj=False
        )
        self.assertEqual(inc, self.config1)
        ...
        inc = intensifier._compare_configs(
            challenger=self.config2, incumbent=inc, run_history=self.rh, log_traj=False
        )
        self.assertEqual(inc, self.config1)
        self.rh.add(
            config=self.config2,
        ...
        inc = intensifier._compare_configs(
            challenger=self.config3, incumbent=inc, run_history=self.rh, log_traj=False
        )
        self.assertEqual(inc, self.config1)
        self.rh.add(
            config=self.config3,
        ...
        inc = intensifier._compare_configs(
            challenger=self.config4, incumbent=inc, run_history=self.rh, log_traj=False
        )
        self.assertEqual(inc, self.config3)
        self.rh.add(
            config=self.config4,
    ...
    def test_incumbent_selection_designs(self):
        ...
        inc = intensifier._compare_configs(
            incumbent=self.config4,
            challenger=self.config3,
            run_history=self.rh,
            log_traj=False,
        )
    ...
    def _exhaust_stage_execution(self, intensifier, taf, challengers, incumbent):
        ...
        while True:
            ...
            if toggle.pop():
                result = eval_challenger(
                    run_info, taf, self.stats, self.rh, force_update=True
                )
    ...
    def test_iteration_done_only_when_all_configs_processed_instance_as_budget(self):
        ...
        challengers = [self.config1, self.config2, self.config3, self.config4]
        incumbent = None
        pending_processing, incumbent = self._exhaust_stage_execution(
            intensifier, taf, challengers, incumbent
        )
    ...
    def test_budget_initialization(self):
        ...
        for minb, maxb, eta, n_configs_in_stage, all_budgets in to_check:
            ...
            np.testing.assert_array_almost_equal(
                n_configs_in_stage, comp_configs, decimal=5
            )
