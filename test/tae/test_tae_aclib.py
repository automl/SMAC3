...
class TaeOldTest(unittest.TestCase):
    ...
    def test_run(self):
        ...
        '''
        eta = ExecuteTARunAClib(ta=shlex.split("python test/tae/dummy_ta_wrapper_aclib.py 1"))
        ...
        eta = ExecuteTARunAClib(ta=shlex.split("python test/tae/dummy_ta_wrapper_aclib.py 2"))
        status, cost, runtime, ar_info = eta.run(config={})
        ...
        eta = ExecuteTARunAClib(ta=shlex.split("python test/tae/dummy_ta_wrapper_aclib.py 2"), run_obj="quality")
        status, cost, runtime, ar_info = eta.run(config={},)
        assert status == StatusType.SUCCESS
        assert cost == 2.0
        assert runtime == 3.0
