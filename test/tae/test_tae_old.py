...
class TaeOldTest(unittest.TestCase):
    ...
    def test_run(self):
        ...
        '''
        eta = ExecuteTARunOld(
            ta=shlex.split("python test/tae/dummy_ta_wrapper.py 1"))
        ...
        eta = ExecuteTARunOld(
            ta=shlex.split("python test/tae/dummy_ta_wrapper.py 2"))
        status, cost, runtime, ar_info = eta.run(config={})
        assert status == StatusType.SUCCESS
        assert cost == 2.0
        assert runtime == 2.0
        ...
        eta = ExecuteTARunOld(
            ta=shlex.split("python test/tae/dummy_ta_wrapper.py 2"), run_obj="quality")
        status, cost, runtime, ar_info = eta.run(config={},)
        assert status == StatusType.SUCCESS
        assert cost == 4.0
        assert runtime == 2.0
