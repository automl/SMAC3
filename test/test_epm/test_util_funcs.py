...
class TestUtilFuncs(unittest.TestCase):
    ...
    def test_check_points_in_ss(self):
        ...
        np.testing.assert_equal(check_points_in_ss(X_cont,
                                                   cont_dims=cont_dims,
        ...
        np.testing.assert_equal(check_points_in_ss(X_cat, cat_dims=cat_dims, bounds_cat=bounds_cat),
                                [True, True, False])

        # cat + cont
        X_mix = np.hstack([X_cont, X_cat])
        cat_dims += len(cont_dims)
        ss_mix = check_points_in_ss(X_mix,
                                    cont_dims=cont_dims,
                                    cat_dims=cat_dims,
                                    bounds_cont=bounds_cont,
