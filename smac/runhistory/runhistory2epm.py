...
class AbstractRunHistory2EPM(object):
    ...
    def transform(self, runhistory: RunHistory):
        ...
        # Also get CRASHED runs
        c_run_dict = {run: runhistory.data[run] for run in runhistory.data.keys()
                      if runhistory.data[run].status == StatusType.CRASHED}
    
        c_instance_id_list = [k.instance_id for k in c_run_dict.keys()]
        cX, _ = self._build_matrix(run_dict=c_run_dict, runhistory=runhistory,
                                  instances=c_instance_id_list, store_statistics=True)
        ...
        if cX.shape[0] == 0:
            ...
            feasible = np.hstack((np.ones(X.shape[0], dtype=bool), np.zeros(cX.shape[0], dtype=bool)))
            cX = np.vstack((X, cX))
    
        self.logger.debug("Converted %d observations" % (X.shape[0]))
        return X, Y, cX, feasible
