...
def _unbound_tae_starter(
    tae: BaseRunner, runhistory: typing.Optional[RunHistory], run_info: RunInfo, *args: typing.Any, **kwargs: typing.Any
) -> RunValue:
    """
...
class Validator(object):
        ...
        for idx, run in enumerate(runs):
            if self.scen.feature_array is not None and run.inst is not None:
                X_pred[idx] = np.hstack(
                    [convert_configurations_to_array([run.config])[0], self.scen.feature_dict[run.inst]]
                )
            else:
    ...
    def _get_runs(
        ...
        if self.scen.deterministic and repetitions != 1:  # type: ignore[attr-defined] # noqa F821
            self.logger.warning(
                "Specified %d repetitions, but fixing to 1, " "because algorithm is deterministic.", repetitions
            )
            repetitions = 1
    ...
    def _process_runhistory(
        ...
        if runhistory:
            ...
            return {
                i: sorted(
                    [(seed, list(inst_seed_config[i][seed])) for seed in inst_seed_config[i]], key=lambda x: len(x[1])
                )
