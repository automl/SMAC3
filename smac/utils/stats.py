...
class Stats:
    ...
    def get_remaining_ta_runs(self) -> int:
        ...
        if self.config:
            return self.config.n_runs - self.submitted_ta_runs  # type: ignore[attr-defined] # noqa F821
        else:
            raise ValueError("Scenario is missing")
    ...
    def get_remaining_ta_budget(self) -> float:
        ...
        if self.config:
            return self.config.cputime_limit - self.ta_time_used
        else:
            raise ValueError("Scenario is missing")
    ...
    def print(self, debug_out: bool = False) -> None:
        ...
        log_func(
            ...
            % (self.submitted_ta_runs, str(self.config.n_runs))  # type: ignore[attr-defined] # noqa F821
        )
        log_func(
            "Finished target algorithm runs: %d / %s"
            % (self.finished_ta_runs, str(self.config.n_runs))  # type: ignore[attr-defined] # noqa F821
        )
        log_func("Configurations: %d" % (self.n_configs))
        log_func("Used wallclock time: %.2f / %.2f sec " % (time.time() - self._start_time, self.config.walltime_limit))
        log_func("Used target algorithm runtime: %.2f / %.2f sec" % (self.ta_time_used, self.config.cputime_limit))
