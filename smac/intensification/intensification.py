...
class Intensifier(AbstractRacer):
    ...
    def process_results(
        ...
        *   The first run results need to be processed
        ...
        if self.stage in [IntensifierStage.PROCESS_INCUMBENT_RUN, IntensifierStage.PROCESS_FIRST_CONFIG_RUN]:
    ...
    def _process_inc_run(
        ...
        if self.stage in [IntensifierStage.RUN_FIRST_CONFIG, IntensifierStage.PROCESS_FIRST_CONFIG_RUN]:
            self.stage = IntensifierStage.RUN_INCUMBENT
            self._next_iteration()
        else:
    ...
    def _process_racer_results(
        ...
        if not self.to_run:
            new_incumbent = self._compare_configs(
                incumbent=incumbent, challenger=challenger, run_history=run_history, log_traj=log_traj
            )
            ...
            if new_incumbent == incumbent:
                ...
                self.continue_challenger = False
                self.logger.debug(
                    "Estimated cost of challenger on %d runs: %.4f, but worse than incumbent", len(chal_runs), chal_perf
                )
                ...
                if self.always_race_against and self.always_race_against != challenger:
                    ...
                    self.stage = IntensifierStage.RUN_INCUMBENT
                self.logger.debug(
                    "Estimated cost of challenger on %d runs: %.4f, becomes new incumbent", len(chal_runs), chal_perf
                )
    ...
    def _generate_challengers(
        self, challengers: typing.Optional[typing.List[Configuration]], chooser: typing.Optional[EPMChooser]
    ) -> _config_to_run_type:
        """
