...
def test_race_challenger_large(make_scenario, make_stats, configspace_small, runhistory):
    ...
    while True:
        if intensifier.continue_challenger:
            ...
            config, _ = intensifier.get_next_challenger(challengers=[configs[1], configs[2]], chooser=None)
...
def testget_next_challenger(make_scenario, make_stats, configspace_small, runhistory):
    ...
    config, new = intensifier.get_next_challenger(challengers=[configs[0], configs[1]], chooser=None)
    ...
    config, new = intensifier.get_next_challenger(challengers=[configs[1]], chooser=None)
    assert config == configs[0] == intensifier.current_challenger
    assert intensifier._challenger_id, 1
    assert not new
...
def test_generate_challenger(make_scenario, make_stats, configspace_small, runhistory):
    ...
    gen = intensifier._generate_challengers(challengers=[configs[0], configs[1]], chooser=None)
    ...
    gen = intensifier._generate_challengers(challengers=None, chooser=smac.configuration_chooser)
...
def test_evaluate_challenger_3(self):
    ...
    taf = TargetAlgorithmRunner(use_pynisher=False, ta=target, stats=stats, run_obj="quality")
    taf.runhistory = runhistory
    ...
    assert intensifier.stage, IntensifierStage.RUN_CHALLENGER)
    assert 
        len(runhistory.get_runs_for_config(configs[0], only_max_observed_budget=True)),
        2,
    )
...
def test_no_new_intensification_wo_challenger_run(self):
    ...
    intent, run_info = intensifier.get_next_run(
        challengers=[configs[2]],
        incumbent=None,
        runhistory=runhistory,
        chooser=None,
    )
    assert run_info.config= configs[2]
    assert intensifier.stage, IntensifierStage.PROCESS_FIRST_CONFIG_RUN)
    run_value = evaluate_challenger(run_info, target_algorithm, stats, runhistory)
    inc, perf = intensifier.process_results(
        run_info=run_info,
    ...
    intent, run_info = intensifier.get_next_run(
        challengers=[configs[2]],  # since incumbent is run, no configs required
        incumbent=inc,
        runhistory=runhistory,
        chooser=None,
    )

    assert run_info.config, None)
    assert intensifier.stage, IntensifierStage.RUN_CHALLENGER)
    ...
    intent, run_info = intensifier.get_next_run(
        challengers=[configs[0]], incumbent=inc, runhistory=runhistory, chooser=None
    )
    ...
    intent, run_info = intensifier.get_next_run(
        challengers=[configs[1]], incumbent=inc, runhistory=runhistory, chooser=None
    )
    run_value = evaluate_challenger(run_info, target_algorithm, stats, runhistory)
    inc, perf = intensifier.process_results(
        run_info=run_info,
