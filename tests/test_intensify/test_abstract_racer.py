...
class TestAbstractRacer(unittest.TestCase):
    ...
    def test_compare_configs_no_joint_set(self):
        intensifier = AbstractRacer(
            stats=self.stats, traj_logger=TrajLogger(output_dir=None, stats=self.stats), rng=None, instances=[1]
        )
        ...
        self.rh.add(
            config=self.config2, cost=1, time=1, status=StatusType.SUCCESS, instance_id=1, seed=1, additional_info=None
        )
        conf = intensifier._compare_configs(incumbent=self.config1, challenger=self.config2, run_history=self.rh)
        self.assertIsNone(conf)
