...
class TestPSMAC(unittest.TestCase):
    ...
    def test_write(self):
        ...
        fixture = (
            '{"data": [[[1, "branin", 1, 0.0], [1, 1, {"__enum__": '
        ...
        run_history.add(config, 1, 1, StatusType.SUCCESS, seed=1, instance_id="branin")
        run_history.add(config, 1, 1, StatusType.SUCCESS, seed=1, instance_id="branini")
        config_2 = configuration_space.sample_configuration()
        # Another config on a known instance
        run_history.add(config_2, 1, 1, StatusType.SUCCESS, seed=1, instance_id="branini")
        ...
        run_history.add(config_3, 1, 1, StatusType.SUCCESS, seed=1, instance_id="branin-hoo")
        ...
        pSMAC.read(
            run_history=run_history, output_dirs=[self.tmp_dir], configuration_space=configuration_space, logger=logger
        )
        self.assertEqual(r_size, len(run_history.data), "Runhistory should be the same and not changed after reading")
    ...
    def test_load(self):
        ...
        other_runhistory_filename = os.path.join(self.tmp_dir, "runhistory.json")
