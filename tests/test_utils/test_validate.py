...
class ValidationTest(unittest.TestCase):
    def setUp(self):
        ...
        self.output_files = [
            self.output_rh + "validated_runhistory_EPM.json",
    ...
    def test_get_runs_capped(self):
        ...
        during rh-recovery"""
        scen = Scenario(self.scen_fn, cmd_options={"run_obj": "quality", "instances": ["0"]})
        ...
        expected = [
            _Run(inst_specs="0", seed=0, inst="0", config=old_configs[2]),
    ...
    def test_get_runs(self):
        ...
        self.maxDiff = None
        expected = [
            _Run(config="config1", inst="3", seed=1608637542, inst_specs="three"),
        ...
        # Only train
        expected = [
            _Run(config="config1", inst="0", seed=423734972, inst_specs="null"),
        ...
        # Test and train
        expected = [
            _Run(config="config1", inst="0", seed=249467210, inst_specs="null"),
    ...
    def test_validate(self):
        ...
        self.assertEqual(
            sum([len(rh.get_runs_for_config(c, only_max_observed_budget=True)) for c in rh.get_all_configs()]), 21
        )
    ...
    def test_validate_no_insts(self):
        ...
        self.assertEqual(
            sum([len(rh.get_runs_for_config(c, only_max_observed_budget=True)) for c in rh.get_all_configs()]), 6
        )

    def test_validate_deterministic(self):
        """deterministic ta"""
        scen = Scenario(
            self.scen_fn, cmd_options={"run_obj": "quality", "train_insts": self.train_insts, "deterministic": True}
        )
        scen.instance_specific = self.inst_specs
        validator = Validator(scen, self.trajectory, self.rng)
    ...
    def test_passed_runhistory(self):
        ...
        for config in old_configs[: int(len(old_configs) / 2)]:
            old_rh.add(config, 1, 1, StatusType.SUCCESS, instance_id="0", seed=seeds[old_configs.index(config)])

        configs = validator._get_configs("all")
        insts = validator._get_instances("train")
        runs_w_rh = validator._get_runs(configs, insts, repetitions=2, runhistory=old_rh)
    ...
    def test_objective_runtime(self):
        ...
        scen = Scenario(self.scen_fn, cmd_options={"run_obj": "runtime", "cutoff_time": 5})
        validator = Validator(scen, self.trajectory, self.rng)
        old_configs = [entry["incumbent"] for entry in self.trajectory]
        old_rh = RunHistory()
        for config in old_configs[: int(len(old_configs) / 2)]:
    ...
    def test_inst_no_feat(self):
        ...
        scen = Scenario(
            self.scen_fn,
            cmd_options={"run_obj": "quality", "train_insts": self.train_insts, "test_insts": self.test_insts},
        )
