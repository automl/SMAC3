...
class TestSerialRunner(unittest.TestCase):
    ...
    def test_run(self):
        ...
        run_info = RunInfo(
            config=2, instance="test", instance_specific="0", seed=0, cutoff=None, capped=False, budget=0.0
        )
    ...
    def test_serial_runs(self):
        ...
        runner.submit_run(run_info)
        run_info = RunInfo(
            config=3, instance="test", instance_specific="0", seed=0, cutoff=None, capped=False, budget=0.0
        )
        runner.submit_run(run_info)
        run_values = runner.get_finished_runs()
