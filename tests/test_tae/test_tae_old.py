...
class TaeOldTest(unittest.TestCase):
    ...
    def test_run(self):
        ...
        eta = ExecuteTARunOld(ta=shlex.split("python test/test_tae/dummy_ta_wrapper.py 1"), stats=stats)
        ...
        eta = ExecuteTARunOld(ta=shlex.split("python test/test_tae/dummy_ta_wrapper.py 2"), stats=stats)
        status, cost, runtime, ar_info = eta.run(config={})
        assert status == StatusType.SUCCESS
        assert cost == 2.0
        assert runtime == 2.0
        ...
        eta = ExecuteTARunOld(
            ta=shlex.split("python test/test_tae/dummy_ta_wrapper.py 2"), stats=stats, run_obj="quality"
        )
