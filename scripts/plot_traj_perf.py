...
def setup_SMAC_from_file(smac_out_dn: str, add_dn: typing.List[str]):
    """
    ...
    traj = TrajLogger.read_traj_aclib_format(fn=os.path.join(smac_out_dn, "traj_aclib2.json"), cs=scenario.cs)
...
def predict_perf_of_traj(traj, smac: AlgorithmConfiguration):
    ...
    logger.info("Predict performance of %d entries in trajectory." % (len(traj)))
    time_stamps = []
    perfs = []
    for entry in traj:
        ...
        m, v = smac.solver.model.predict_marginalized_over_instances(X=config_array)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--smac_output_dir", required=True, help="Output directory of SMAC")
    parser.add_argument(
        "--additional_data",
        nargs="*",
    ...
    smac, traj = setup_SMAC_from_file(smac_out_dn=args_.smac_output_dir, add_dn=args_.additional_data)
