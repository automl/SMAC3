...
from smac.runner.execute_ta_run_aclib import ExecuteTARunAClib  # noqa: E402
from smac.runner.execute_ta_run_old import ExecuteTARunOld  # noqa: E402
...
if __name__ == "__main__":
    ...
    req_opts.add_argument("--scenario", required=True, help="path to SMAC scenario")
    req_opts.add_argument("--trajectory", required=True, help="path to SMAC trajectory")
    req_opts.add_argument("--output", required=True, help="path to save runhistory to")
    ...
    req_opts.add_argument(
        ...
        default="def+inc",
        type=str,
        choices=["def", "inc", "def+inc", "wallclock_time", "cpu_time", "all"],
    ...
    req_opts.add_argument("--epm", dest="epm", action="store_true", help="Use EPM to validate")
    req_opts.add_argument("--no-epm", dest="epm", action="store_false", help="Don't use EPM to validate")
    ...
    req_opts.add_argument(
        ...
        default=1,
        type=int,
        help="number of repetitions for nondeterministic " "algorithms",
    )
    req_opts.add_argument("--n_jobs", default=1, type=int, help="number of cpu-cores to use (-1 to use all)")
    req_opts.add_argument(
        "--tae",
        default="old",
        type=str,
        help="what tae to use (if not using epm)",
        choices=["aclib", "old"],
    )
    req_opts.add_argument("--verbose_level", default="INFO", choices=["INFO", "DEBUG"], help="verbose level")

    args_, misc = parser.parse_known_args()

    # remove leading '-' in option names
    misc = dict((k.lstrip("-"), v.strip("'")) for k, v in zip(misc[::2], misc[1::2]))
    ...
    if args_.epm:
        validator.validate_epm(
            config_mode=args_.configs,
