...
class TestSMACFacade(unittest.TestCase):
    ...
    def test_construct_runhistory(self):
        ...
        smbo = AlgorithmConfigurationFacade(self.scenario, runhistory_kwargs={"overwrite_existing_runs": True})
        self.assertIsInstance(smbo.solver.runhistory, RunHistory)
        self.assertTrue(smbo.solver.runhistory.overwrite_existing_runs)
        smbo = AlgorithmConfigurationFacade(self.scenario, runhistory=RunHistory)
        self.assertIsInstance(smbo.solver.runhistory, RunHistory)
    ...
    def test_construct_random_configuration_chooser(self):
        ...
        smbo = AlgorithmConfigurationFacade(self.scenario, random_configuration_chooser_kwargs={"rng": rng})
        self.assertIsInstance(smbo.solver.epm_chooser.random_configuration_chooser, ChooserProb)
        self.assertIs(smbo.solver.epm_chooser.random_configuration_chooser.rng, rng)
        smbo = AlgorithmConfigurationFacade(self.scenario, random_configuration_chooser_kwargs={"prob": 0.1})
        self.assertIsInstance(smbo.solver.epm_chooser.random_configuration_chooser, ChooserProb)
        ...
        with self.assertRaisesRegex(Exception, "got an unexpected keyword argument"):
            AlgorithmConfigurationFacade(self.scenario, random_configuration_chooser_kwargs={"dummy": 0.1})
    ...
    def test_construct_epm(self):
        ...
        # Check for construction failure on wrong argument
        with self.assertRaisesRegex(Exception, "got an unexpected keyword argument"):
            AlgorithmConfigurationFacade(self.scenario, model_kwargs={"dummy": 0.1})
    ...
    def test_construct_acquisition_function(self):
        ...
        # Check for construction failure on wrong argument
        with self.assertRaisesRegex(Exception, "got an unexpected keyword argument"):
            AlgorithmConfigurationFacade(self.scenario, acquisition_function_kwargs={"dummy": 0.1})
    ...
    def test_construct_intensifier(self):
        ...
        smbo = AlgorithmConfigurationFacade(self.scenario, intensifier_kwargs={"maxR": 987})
        ...
        smbo = AlgorithmConfigurationFacade(self.scenario, intensifier=dummy_intensifier)
    ...
    def test_init_EIPS_as_arguments(self):
        for objective in ["runtime", "quality"]:
            ...
            with self.assertRaisesRegex(TypeError, "the surrogate model must support multi-objective prediction!"):
                AlgorithmConfigurationFacade(
                    self.scenario, acquisition_function=EIPS, runhistory2epm=RunHistory2EPM4EIPS
                )

    ####################################################################################################################
    # Other tests...

    @unittest.mock.patch.object(AlgorithmConfigurationFacade, "__init__")
    ...
    def test_get_runhistory_and_trajectory_and_tae_runner(self):
        ...
        smac = AlgorithmConfigurationFacade(tae_runner=func, scenario=self.scenario)
    ...
    def test_output_structure(self):
        ...
        smac = AlgorithmConfigurationFacade(scenario=scen1, run_id=1)
        ...
        smac2 = AlgorithmConfigurationFacade(scenario=scen1, run_id=1)
        self.assertTrue(os.path.isdir(smac2.output_dir + ".OLD"))

        smac3 = AlgorithmConfigurationFacade(scenario=scen1, run_id=1)
        self.assertTrue(os.path.isdir(smac3.output_dir + ".OLD.OLD"))

        smac4 = AlgorithmConfigurationFacade(scenario=scen1, run_id=2)
    ...
    def test_register_callback(self):
        smac = AlgorithmConfigurationFacade(scenario=self.scenario, run_id=1)
    ...
    def test_set_limit_resources_with_tae_func_dict(self):
        ...
        smac = AlgorithmConfigurationFacade(scenario=scenario, tae_runner=tmp, rng=1)
