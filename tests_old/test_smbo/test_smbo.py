...
from smac.algorithm_executer import FirstRunCrashedException, StatusType
...
class TestSMBO(unittest.TestCase):
    ...
    def test_rng(self):
        smbo = AlgorithmConfigurationFacade(self.scenario, rng=None).solver
        self.assertIsInstance(smbo.rng, np.random.RandomState)
        self.assertIsInstance(smbo.num_run, int)
        smbo = AlgorithmConfigurationFacade(self.scenario, rng=1).solver
        rng = np.random.RandomState(1)
        self.assertEqual(smbo.num_run, 1)
        self.assertIsInstance(smbo.rng, np.random.RandomState)
        smbo = AlgorithmConfigurationFacade(self.scenario, rng=rng).solver
        self.assertIsInstance(smbo.num_run, int)
    ...
    def test_abort_on_initial_design(self, patch):
        ...
        smbo = AlgorithmConfigurationFacade(scen, tae_runner=target, rng=1).solver
    ...
    def test_stop_smbo(self, patch):
        ...
        self.output_dirs.append(scen.output_dir)
        smbo = AlgorithmConfigurationFacade(scen, tae_runner=target, rng=1)
    ...
    def test_intensification_percentage(self):
        ...
        def get_smbo(intensification_perc):
            ...
            self.output_dirs.append(scen.output_dir)
            return AlgorithmConfigurationFacade(scen, tae_runner=target, rng=1).solver
    ...
    def test_update_intensification_percentage(self):
        ...
        self.output_dirs.append(scen.output_dir)
        solver = AlgorithmConfigurationFacade(scen, tae_runner=target, rng=1).solver
    ...
    def test_validation(self):
        with mock.patch.object(TrajLogger, "read_traj_aclib_format", return_value=None):
            ...
            smac = AlgorithmConfigurationFacade(self.scenario)
    ...
    def test_incorporate_run_results_callback_stop_loop(self, process_results_mock):
        ...
        self.scenario.output_dir = None
        smac = AlgorithmConfigurationFacade(self.scenario, tae_runner=target, rng=1)
        smac.register_callback(callback)
