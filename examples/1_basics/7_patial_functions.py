"""
Using partial functions as a target algorithm
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
This is rather an edge case, but it is possible to use partial functions as a target algorithm.
Let's assume you had a target algorithm for which you want to prefix some arguments,
because you want to reuse it for e.g. different datasets. This example is an adaptation of 6_priors.py.
But be aware of the following limitations, that partial brings along:
https://docs.python.org/2/library/functools.html#partial-objects. In particular,
a partial function derived from a class method will behave as a static method.
...
if __name__ == '__main__':
    ...
    scenario = Scenario(configspace(), n_trials=40)
